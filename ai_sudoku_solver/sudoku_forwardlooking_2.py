    ...
    for index, cell in enumerate(pseudo_puzzle):
        for constraint_set in constraint_sets:
            for constrained_indices in constraint_set:
                if index in constrained_indices: neighbors[index] |= constrained_indices
    return neighbors


def forward_looking(state, solved, neighbors_dictionary):
    visited = set(solved)
    while len(solved) > 0:
        for index in neighbors_dictionary[solved[-1]]:
            if index in solved or index not in visited:
                if index != solved[-1]: state[index] = state[index].replace(state[solved[-1]], "")
                if index not in visited and len(state[index]) == 1:
                    solved.append(index)
                    visited.add(index)
            if len(state[index]) == 0: return None
        solved.remove(solved[-1])
    return state


def count_occurrences(state, constraint_set, value):
    count = 0
    if value not in constraint_set: return count
    for index in constraint_set:
        if state[index] == value: count += 1
    return count
...
def constraint_propagation(state, constraint_sets):
    for constraint_set in constraint_sets:
        for index in constraint_set:
            num_occurrences = count_occurrences(state, constraint_set, state[index])
            if num_occurrences == 0: return None
    return state
...
def csp_backtracking_with_forward_looking(state, solved, pseudo_puzzle_size, neighbors_dictionary):
    ...
    for val in get_sorted_values(state, var):
        ...
        new_state[var] = val
        checked_state = forward_looking(new_state, [var], neighbors_dictionary)

        if checked_state is not None:
            result = csp_backtracking_with_forward_looking(checked_state, solved, pseudo_puzzle_size, neighbors_dictionary)
...
# pseudo_puzzle = "3.......7....6..2..5..3....2..7........4..5........21.6.....3.8.....1......5....."
# pseudo_puzzle = "5......8....31............2.1....3.....5.8....4..........6..14.....3.7..8.2......"
...
#
# checked_puzzle = forward_looking(puzzle, solved, neighbors_dictionary)
# solution = csp_backtracking_with_forward_looking(checked_puzzle, solved, size, neighbors_dictionary)
# print("Puzzle:")
