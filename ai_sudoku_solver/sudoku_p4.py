...
def constraint_propagation(state, var, neighbors, constraint_sets, symbol_set):
    changes_made = False
    for constraint_set in constraint_sets:
        for val in symbol_set:
            ...
            if len(val_indices) == 1:
                ...
                changes_made = True
    if changes_made: state = forward_looking(state, [var], neighbors)
    return state


def csp_backtracking(state, neighbors, symbol_set, constraint_sets, size):
    var = get_most_constrained_var(state, size)
    if var == -1: return state
    for val in state[var]:
        new_state = state.copy()
        new_state[var] = val
        checked_state = forward_looking(new_state, [var], neighbors)
        if checked_state is not None:
            propagated_state = constraint_propagation(checked_state, var, neighbors, constraint_sets, symbol_set)
            if propagated_state is not None:
                result = csp_backtracking(propagated_state, neighbors, symbol_set, constraint_sets, size)
...
#     print(i, "Puzzle:", puzzle)
#     print(i, "Solution:", get_solution(puzzle))
#     print()

# print_possibilities(puzzles[99], get_size(puzzles[99]))
